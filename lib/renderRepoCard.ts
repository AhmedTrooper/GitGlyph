...
import { PADDING, RESPONSIVE_STYLE_CSS, svgRootAttrs } from './svgBadge';
...
/**
 * Width of the right-side "Public/Fork" badge.
 */
...
export function renderRepoCard(repo: RepoDetails, options: RenderRepoOptions = {}): string {
  ...
  const rawTitle = options.customTitle || repo.name;

  const width = 450;
  const height = 140;
  // Right-side badge sits flush against the right padding
  const badgeX = width - PADDING - BADGE_WIDTH;
  // Title has limited horizontal space: from PADDING (icon) to badgeX - small gap
...
<svg ${svgRootAttrs(width, height, options.requestedWidth)}>
  ...
  <g transform="translate(${PADDING}, 25)">
    <svg class="icon" viewBox="0 0 16 16" width="16" height="16" y="2">
      <path d="${ICONS.repo}"/>
    </svg>
    <text x="24" y="15" class="repo-title">${escapeXml(truncateText(rawTitle, titleMaxChars))}</text>

    <g transform="translate(${badgeX - PADDING}, 0)">
      <rect width="${BADGE_WIDTH}" height="18" rx="9" fill="${theme.badgeBg}" stroke="${theme.border}"/>
      <text x="${BADGE_WIDTH / 2}" y="12.5" text-anchor="middle" class="badge">${badgeLabel}</text>
    </g>
  </g>

  <text x="${PADDING}" y="68" class="description">${description}</text>

  <g transform="translate(${PADDING}, 105)">
    <g>
